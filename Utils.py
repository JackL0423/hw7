...
def decode_token_char(token):
    return str(chr(max(32, token)))

def decode_tokens_char(tokens):
...
def get_loaders_enwiki8_basedon_transformerXL(seq_len, batch_size):
    ...
    parser.add_argument('--dataset', type=str, default='enwiki8',
                        choices=['wt103', 'lm1b', 'enwik8', 'text8'],
                        help='dataset name')
    parser.add_argument('--split', type=str, default='train',
                        choices=['train', 'valid', 'text'],
...
def get_loaders_text8(seq_len, batch_size):
    """Prepare text8 data"""
    file_train = open('./data/text8files/train.txt', 'r')
